...
fn inst_container(id: &str, isolated: bool) -> std::path::PathBuf {
    if isolated {
        for (d, vid) in scan_instance_dirs() {
            if vid == id {
...
fn dedup_disp(s: &str) -> String {
    ...
    for n in 1..words.len() {
        ...
        let tail = words[n..].join(" ");
        if tail.starts_with(&head) && tail.len() > head.len() {
            return tail;
        }
    }
    s.to_string()
...
impl MinecraftView {
    ...
    pub fn new(state: &AppState, parent: &adw::ApplicationWindow) -> Self {
        ...
        main_stack.set_valign(gtk::Align::Fill);
        main_stack.set_hexpand(true);

        // ============ LIBRARY PAGE (Carbon HomeGrid) ============
        let lib_page = gtk::Box::new(gtk::Orientation::Vertical, 16);
        ...
        add_compact.add_css_class("add-btn");
        add_compact.set_visible(false);
        add_compact.set_valign(gtk::Align::Center);
        ...
        if let Ok(c1100) = adw::BreakpointCondition::parse("max-width: 1100px") {
            let bp = adw::Breakpoint::new(c1100);
            let t = gtk::glib::Value::from(true);
            let f = gtk::glib::Value::from(false);
            bp.add_setter(&sort_drop, "visible", Some(&f));
            bp.add_setter(&sort_cycle, "visible", Some(&t));
            parent.add_breakpoint(bp);
        }
        if let Ok(c800) = adw::BreakpointCondition::parse("max-width: 800px") {
            let bp2 = adw::Breakpoint::new(c800);
            let t = gtk::glib::Value::from(true);
            let f = gtk::glib::Value::from(false);
            bp2.add_setter(&account_label, "visible", Some(&f));
            bp2.add_setter(&add_btn, "visible", Some(&f));
            bp2.add_setter(&add_compact, "visible", Some(&t));
            parent.add_breakpoint(bp2);
        }
        ...
        ad_page.set_margin_top(24);
        ad_page.set_margin_bottom(24);
        ad_page.set_margin_start(24);
        ad_page.set_margin_end(24);
        let ad_toolbar = gtk::Box::new(gtk::Orientation::Horizontal, 6);
        ad_toolbar.add_css_class("mcx-bar");
        ...
        addons_pop_box.set_margin_top(6);
        addons_pop_box.set_margin_bottom(6);
        addons_pop_box.set_margin_start(6);
        addons_pop_box.set_margin_end(6);
        let rescan_btn = themed_btn("search", "Rescan", state.theme.is_dark(), 14);
        rescan_btn.add_css_class("settings-btn");
        rescan_btn.set_halign(gtk::Align::Fill);
        addons_pop_box.append(&rescan_btn);
        let ad_folder_btn = gtk::Button::with_label("Open folder");
        ad_folder_btn.add_css_class("settings-btn");
        ad_folder_btn.set_halign(gtk::Align::Fill);
        addons_pop_box.append(&ad_folder_btn);
        ...
        lg_page.set_margin_top(24);
        lg_page.set_margin_bottom(24);
        lg_page.set_margin_start(24);
        lg_page.set_margin_end(24);
        ...
        st_page.set_margin_top(24);
        st_page.set_margin_bottom(24);
        st_page.set_margin_start(24);
        st_page.set_margin_end(24);
        ...
        {
            let v = view.clone();
            add_compact.connect_clicked(move |_| v.show_add_instance());
        }
        {
            let v = view.clone();
    ...
    fn refresh_all(&self) {
        ...
        for _ in 0..4 {
            let sk = gtk::FlowBoxChild::new();
            let inner = gtk::Box::new(gtk::Orientation::Vertical, 6);
            inner.add_css_class("skeleton-tile");
            sk.set_child(Some(&inner));
            self.flow.insert(&sk, -1);
        }
    ...
    fn render_library(&self) {
        ...
        let msg = if list.is_empty() {
            if total == 0 {
                ...
                "Instances · no matches".to_string()
            }
        } else if list.len() == total {
            format!("Instances · {} {}", total, if total == 1 { "instance" } else { "instances" })
        } else {
            format!("Instances · {} of {}", list.len(), total)
        };
        self.lib_status.set_text(&msg);
        self.lib_status.add_css_class("frame-title");
        if list.is_empty() && total == 0 {
            let empty = gtk::Box::new(gtk::Orientation::Vertical, 10);
            empty.add_css_class("mc-empty");
            empty.set_halign(gtk::Align::Center);
            empty.set_valign(gtk::Align::Center);
            empty.set_hexpand(true);
            let icon = helpers::themed_image("minecraft", self.state.theme.is_dark(), 56);
            icon.set_halign(gtk::Align::Center);
            empty.append(&icon);
            let lbl = gtk::Label::new(Some("No instances yet"));
            lbl.add_css_class("mc-tile-name");
            lbl.set_halign(gtk::Align::Center);
            empty.append(&lbl);
            let sub = gtk::Label::new(Some("Create your first instance to start playing."));
            sub.add_css_class("mc-tile-sub");
            sub.set_halign(gtk::Align::Center);
            empty.append(&sub);
            let go = gtk::Button::with_label("Add instance");
            go.add_css_class("add-btn");
            go.set_halign(gtk::Align::Center);
            let vv = self.clone();
            go.connect_clicked(move |_| vv.show_add_instance());
            empty.append(&go);
        ...
        for inst in &list {
            let tile = gtk::FlowBoxChild::new();
            tile.set_width_request((self.cfg("McTileSize").parse().unwrap_or(64) + if compact { 110 } else { 134 }).max(158));
            let inner = gtk::Box::new(gtk::Orientation::Vertical, if compact { 4 } else { 6 });
            let pad = if compact { 8 } else { 12 };
            ...
            inner.append(&img);
            let name = gtk::Label::new(Some(&dedup_disp(&inst.disp)));
            name.set_halign(gtk::Align::Center);
            name.set_ellipsize(gtk::pango::EllipsizeMode::Middle);
            name.add_css_class("mc-tile-name");
            inner.append(&name);
            ...
            if let Some(p) = prog {
                ...
                if p < 0.0 {
                    ...
                    bar.set_text(Some(&format!("{}%", (p.clamp(0.0, 1.0) * 100.0) as u32)));
                }
                inner.append(&bar);
            }
            ...
            let vk = self.clone();
            let idk = inst.id.clone();
            let key = gtk::EventControllerKey::new();
            key.connect_key_pressed(move |_, keyval, _, _| {
                if keyval == gtk::gdk::Key::Return || keyval == gtk::gdk::Key::KP_Enter || keyval == gtk::gdk::Key::space {
                    vk.open_detail(&idk);
                    return glib::Propagation::Stop;
                }
                glib::Propagation::Proceed
            });
            tile.add_controller(key);
    ...
    fn open_inst_folder(&self, id: &str) {
        if let Some(inst) = self.get_inst(id) {
            let dir = inst_container(&inst.id, inst.isolated);
    ...
    fn do_repair(&self) {
        ...
        let dir = inst_container(&inst.id, inst.isolated).display().to_string();
    ...
    fn addon_dir(&self) -> std::path::PathBuf {
        ...
        let (dir, isolated) = match self.get_inst(&id) {
            Some(i) => (inst_container(&i.id, i.isolated), i.isolated),
    ...
    fn inst_base_dir(&self) -> std::path::PathBuf {
        ...
        match self.get_inst(&id) {
            Some(i) => inst_container(&i.id, i.isolated),
            None => legacy_dir(),
        }
    }
    ...
    fn render_addons(&self) {
        ...
        let rows: Vec<AddonRow> = self.load_addon_rows().into_iter()
            ...
            .collect();
        let kind_lbl = match self.addons_type.borrow().as_str() {
            "shaders" => "Shaders",
            "resourcepacks" => "Resource Packs",
            _ => "Mods",
        };
        self.addons_count.set_text(&format!("{} {}", kind_lbl, rows.len()));
        ...
        let upd_map: Rc<RefCell<HashMap<String, gtk::Button>>> = Rc::new(RefCell::new(HashMap::new()));
        for r in &rows {
            let row = gtk::Box::new(gtk::Orientation::Horizontal, 8);
            ...
            if !r.project_id.is_empty() {
                ...
                upd_map.borrow_mut().insert(fc2.clone(), upd.clone());
            ...
            let pid3 = r.project_id.clone();
            menu.connect_clicked(move |b| vv.show_addon_menu(b, &fc3, &pid3));
            row.append(&menu);
        ...
        std::thread::spawn(move || {
            let files = ctx.map(|(mdir, mc, loader, at)| {
                MinecraftManager::mod_check_updates(&at, &mc, &loader, &mdir)
                    .ok().and_then(|d| d.get("updates").cloned())
                    .and_then(|v| v.as_array().cloned()).unwrap_or_default()
                    .into_iter().filter_map(|u| u.get("file").and_then(|x| x.as_str()).map(str::to_string))
                    .collect::<Vec<_>>()
            }).unwrap_or_default();
            let _ = tx.send(files);
        });
    ...
    fn show_addon_menu(&self, btn: &gtk::Button, file: &str, project_id: &str) {
        let pop = gtk::Popover::new();
        let box_ = gtk::Box::new(gtk::Orientation::Vertical, 2);
        box_.set_margin_top(6);
        box_.set_margin_bottom(6);
        ...
        for (micon, label, action) in items {
            ...
            b.connect_clicked(move |_| {
                ...
                match act.as_str() {
                    ...
                    "view" => v.show_project_view(&pid, None),
    ...
    fn drive_progress(bar: &gtk::ProgressBar, status: &gtk::Label, stage: Option<String>, percent: Option<f64>, prefix: &str) {
        ...
        match percent {
            Some(pc) => {
                ...
                status.set_text(&format!("{}{} ({}%)", prefix, if st.is_empty() { String::new() } else { format!(" — {}", st) }, pc as u32));
            }
            None => {
                bar.pulse();
